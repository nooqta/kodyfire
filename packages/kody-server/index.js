...
router.get('/', function (req, res) {
  ...
  router.get('/kodies/:id', async (req, res) =>{
    const {LERNA_ROOT_PATH:rootPath} = process.env;
    var {id} = req.params;
    if(fs.existsSync(join(rootPath, 'node_modules', id, 'src/parser/validator/schema.json'))){
      let data = fs.readFileSync(join(rootPath, 'node_modules', id, 'src', 'parser/validator/schema.json'));

...
export class Request implements IConcept {
    ...
    getValidation(model: any, prefix: any = '', relatedModel: any = null): string {
      let validation = '';
      model.fields.forEach((f: any) => {
        if (!relatedModel || (relatedModel != null && f.name != relatedModel.toLowerCase() + '_id')) {
          if (f.options.filter((e: any) => e.key == 'default').length == 0) {
            if (f.options.filter((e: any) => e.key == 'nullable').length == 0) {
              if (f.options.filter((e: any) => e.key == 'unique').length > 0) {
                validation += `'${prefix}${f.name}' => '${this.isEmail(f.name)}required|unique:${this.underscorize(pluralize(model.name))},${prefix}${f.name}',\n`
              } else {
                validation += `'${prefix}${f.name}' => '${this.isEmail(f.name)}required',\n`
              }
            }
          }
        }
      ...
      getRequestValidation(model: any, relationships: any, _prefix: any = ''): string {
        let validation = '';
        validation = this.getValidation(model);
        //Relations validation
        if (model.controller && model.controller != "" && model.controller.with_additional_relations && model.controller.with_additional_relations != "" && model.controller.with_additional_relations.length > 0) {
          model.controller.with_additional_relations.forEach((element: any) => {
            validation += this.getRelationValidation(element, relationships, model.name);
          });
        }
        return validation;
      }
      ...
      getRelationValidation(relation: any, relationships: any, relatedModel: any) {
        let content = `'${this.underscorize(Object.values(relation)[0])}' => 'required',\n`;
        let modelName = '';
        let model = null;
        relationships.forEach((r: any) => {
          if (r.name == Object.values(relation)[0] && r.type == Object.keys(relation)[0]) {
            modelName = r.model
          }
        });
        if (modelName != '') {

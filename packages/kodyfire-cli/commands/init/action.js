...
class Action {
    ...
    static execute(_args = { rootDir: process.cwd() }) {
        return __awaiter(this, void 0, void 0, function* () {
            const { name, dependencies } = yield this.getPackageDependencies(_args.rootDir);
            try {
                const kody = {
                    project: name,
                ...
                if (dependencies.length == 0) {
                    this.displayMessage('No dependencies found. Install kody dependencies first. Use `kody search` to find the dependencies.');
                    process.exit(1);
                }
                for (const dep of dependencies) {
                    ...
                    if (!fs.existsSync((0, path_1.join)(_args.rootDir, 'kody.json'))) {
                        ...
                        const kodyJson = JSON.parse(fs.readFileSync((0, path_1.join)(_args.rootDir, 'kody.json')));
                        kodyJson.sources.push({
                            name: dep.replace('-kodyfire', ''),
                            filename: `kody-${dep.replace('-kodyfire', '')}.json`,
                        });
                        fs.writeFileSync((0, path_1.join)(_args.rootDir, 'kody.json'), JSON.stringify(kodyJson, null, '\t'));
                    }
                }
            }
            catch (error) {
    ...
    static createDefinitionFile(rootDir, dep) {
        return __awaiter(this, void 0, void 0, function* () {
            const filename = (0, path_1.join)(rootDir, `kody-${dep.replace('-kodyfire', '')}.json`);
    ...
    static getEntries(rootDirectory, dep) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { schema } = yield (_a = `${rootDirectory}/node_modules/${dep}`, Promise.resolve().then(() => __importStar(require(_a))));
    ...
    static getPackageDependencies(rootDir = process.cwd()) {
        return __awaiter(this, void 0, void 0, function* () {
            const packageJsonFile = fs.readFileSync((0, path_1.join)(rootDir, 'package.json'));
    ...
    static getDependencyConcepts(dependency, rootDir = process.cwd()) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (!fs.existsSync((0, path_1.join)(rootDir, 'node_modules', dependency))) {
                    this.displayMessage(`${dependency} does not exist. Install it first.`);
                    process.exit(1);
                    // @todo: try a globally installed kody
                    kodyPath = (0, path_1.join)(this.getNpmGlobalPrefix(), 'lib', 'node_modules', dependency);
                }
                let { schema } = yield (_a = kodyPath, Promise.resolve().then(() => __importStar(require(_a))));
                ...
                const templatesPath = (0, path_1.join)(process.cwd(), '.kody', dependency);
                if (fs.existsSync(templatesPath)) {
                    const _schema = yield (_b = (0, path_1.join)(templatesPath, 'schema'), Promise.resolve().then(() => __importStar(require(_b))));
                    if (_schema.schema) {
    ...
    static addConcept(dependency, concept, data, rootDir = process.cwd()) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let content = JSON.parse(fs.readFileSync((0, path_1.join)(rootDir, `kody-${dependency.replace('-kodyfire', '')}.json`), 'utf8'));
                content[concept] = [...content[concept], data];
                content = JSON.stringify(content, null, '\t');
                fs.writeFileSync((0, path_1.join)(rootDir, `kody-${dependency.replace('-kodyfire', '')}.json`), content);
            }
            catch (error) {
                this.displayMessage(error.message);
            }

    ...
    static execute(_args) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!currentKody) {
                    this.displayMessage(`😞 The kody ${kody} is not installed.\nUse kody list to check your installed kodies`, 'red');
                    process.exit(1);
                }
                ...
                if (!['json', 'yaml', 'plantuml'].includes(type)) {
                    if (exports.supportedTypes.includes(type)) {
                        this.displayMessage(`${type} parser not implemented yet. Please use yaml or plantuml (class diagram) for now.`, 'red');
                    }
                    else {
                        this.displayMessage(`Unsupported type ${type}. Supported types are ${exports.supportedTypes.join(', ')}. Only json is implemented for now.`, 'red');
                    }
                    process.exit(1);
                }
                ...
                switch (type) {
                    case 'yaml': {
                        ...
                        imports = Object.keys(yaml.models).map(name => (Object.assign({ name }, yaml.models[name])));
                        break;
                    }
                    case 'plantuml': {
                        const { parse } = require('plantuml-parser');
                        const plantuml = zx_1.fs.readFileSync(_args.source, 'utf8').toString();

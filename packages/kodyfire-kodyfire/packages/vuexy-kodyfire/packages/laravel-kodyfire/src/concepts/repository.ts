...
export class Repository extends  Concept{
    ...
    generate(_data: any) {
        ...
        if (!this.model) {
            const filename = _data.template === 'repositoryParent.php.template' ? 'Repository.php' : 'Repositories.php'
    ...
    hasUplodableMorph(model:any) {    
        return model.relationships.some((r: any) => ['morphOne', 'morphMany'].includes(r.type) && !!r.uploadable)
    }
    ...
    getMethod(relation: any) {
        ...
        switch (Object.keys(relation)[0]) {
            case 'hasMany':
                content = `if(isset($data['${this.underscorize(Object.values(relation)[0])}'])){
                      $model->${Object.values(relation)[0]}()->createMany($data['${this.underscorize(Object.values(relation)[0])}']);
                  }\n`;
                break;
    ...
    getMorphAttachments(model: any) {
        ...
        model.relationships.forEach((element: any) => {
            if (['morphOne', 'morphMany'].includes(element.type) && !!element.uploadable) {
    
                model.fields.forEach((f:any) => {
                    if (f.extra_type == "file" || f.extra_type == "image" || f.extra_type == "video") {
                        attachments += `
    ...
    getAttachments(model: any) {
        ...
        model.fields.forEach((element: any) => {
            if (element.extra_type == "file" || element.extra_type == "image" || element.extra_type == "video") {
                attachments += `

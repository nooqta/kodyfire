...
async function activate(context) {
    ...
    let conceptDisposable = vscode.commands.registerCommand('kodyfire.addConcept', async (args) => {
        try {
            const [dependency, concept] = args.command.arguments;
            const concepts = await getConcepts();
            const dependencyConcepts = concepts.find(d => d.name === dependency &&
                Object.keys(d.concepts?.concepts).find(c => c === concept));
            if (!dependencyConcepts) {
                vscode.window.showErrorMessage(`${dependency} does not have concept ${concept}`);
                return;
            }
            const dependencyConcept = dependencyConcepts.concepts?.concepts[concept];
            let questions = [];
            const conceptNames = Object.keys(dependencyConcept);
            const projectRoot = vscode.workspace.workspaceFolders?.[0].uri.fsPath || '/';
            const schemaDefiniton = InstallAction_1.InstallAction.getSchemaDefinition(dependency, projectRoot);
            for (let i = 0; i < conceptNames.length; i++) {
                const question = await InstallAction_1.InstallAction.conceptToQuestion(conceptNames[i], dependencyConcept[conceptNames[i]], schemaDefiniton);
                if (question) {
                    questions.push(question);
                }
            }
            const answers = await InstallAction_1.InstallAction.prompter(questions);
            InstallAction_1.InstallAction.addConcept(dependency, concept, answers, projectRoot);
        }
        catch (error) {
            InstallAction_1.InstallAction.output.show();
    ...
    let initDisposable = vscode.commands.registerCommand('kodyfire.init', async () => {
        try {
            ...
            if (!InstallAction_1.InstallAction.isWorkspaceFolderOpen()) {
                InitAction_1.InitAction.displayMessage('Please open a workspace folder');
                return;
            }
            if (!(await InstallAction_1.InstallAction.doesPackageJsonExist())) {
                let option = {
            ...
            InitAction_1.InitAction.displayMessage('Project initialized successfully');
        }
        catch (error) {
...
async function getConcepts() {
    ...
    for (let i = 0; i < installedKodies.length; i++) {
        const concept = await InitAction_1.InitAction.getDependencyConcepts(installedKodies[i].name);

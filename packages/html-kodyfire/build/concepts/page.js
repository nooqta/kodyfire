...
class Page {
    ...
    generate(data) {
        return __awaiter(this, void 0, void 0, function* () {
            const template = yield fsPromises.readFile((0, path_1.join)(this.getTemplatesPath(), this.template.path, data.template));
            const tpl = handlebars.compile(template === null || template === void 0 ? void 0 : template.toString());
            const compiled = tpl(data);
            const filename = (0, path_1.join)(this.technology.rootDir, this.outputDir, this.getFilename(data.template));
            // We need to create the directory if it doesn't exist
            yield fsPromises.mkdir((0, path_1.dirname)(filename), { recursive: true });
            yield fsPromises.writeFile(filename, compiled);
        });
    }
    getFilename(name) {
        // const regexp = /(\d+)/;
        return (0, kodyfire_core_1.dasherize)(name.replace('.template', '')
